...
**CRÍTICO**: Las tablas deben crearse en el siguiente orden para evitar errores de dependencias:

1. `campus` (sin dependencias)
2. `profesor` (sin dependencias)
3. `libro` (sin dependencias)
4. `facultad` (depende de: campus, profesor)
5. `programa` (depende de: facultad, profesor)
6. `materia` (depende de: programa)
7. `aula` (depende de: campus)
8. `estudiante` (depende de: programa)
9. `horario` (depende de: materia, profesor, aula)
10. `inscripcion` (depende de: estudiante, materia, horario)
11. `calificacion` (depende de: inscripcion)
12. `prestamo` (depende de: estudiante, libro)

---

## 🗂️ Diccionario de Datos Completo

### 1. Entidad: CAMPUS (Crear PRIMERO - Sin dependencias)
```sql
CREATE TABLE campus (
...
### 2. Entidad: PROFESOR (Crear SEGUNDO - Sin dependencias)
```sql
...
### 3. Entidad: FACULTAD (Crear TERCERO - Depende de: campus, profesor)
```sql
...
### 4. Entidad: PROGRAMA (Crear CUARTO - Depende de: facultad, profesor)
```sql
...
### 5. Entidad: MATERIA (Crear QUINTO - Depende de: programa)
```sql
...
### 6. Entidad: AULA (Crear SEXTO - Depende de: campus)
```sql
CREATE TABLE aula (
...
### 7. Entidad: ESTUDIANTE (Crear SÉPTIMO - Depende de: programa)
```sql
...
### 8. Entidad: LIBRO (Crear OCTAVO - Sin dependencias)
```sql
CREATE TABLE libro (
...
### 9. Entidad: HORARIO (Crear NOVENO - Depende de: materia, profesor, aula)
```sql
CREATE TABLE horario (
    horario_id SERIAL PRIMARY KEY,
    materia_id INTEGER REFERENCES materia(materia_id),
...
### 10. Entidad: INSCRIPCION (Crear DÉCIMO - Depende de: estudiante, materia, horario)
```sql
...
### 11. Entidad: CALIFICACION (Crear UNDÉCIMO - Depende de: inscripcion)
```sql
...
### 12. Entidad: PRESTAMO (Crear DUODÉCIMO - Depende de: estudiante, libro)
```sql
...
-- IMPORTANTE: Ejecutar en el orden especificado para evitar errores de dependencias

-- 1. Crear base de datos
CREATE DATABASE edutech_db;

-- 2. Conectar a la base de datos
-- NOTA: \c es un comando de psql (cliente de PostgreSQL), no SQL estándar
-- Si usas otro cliente, conéctate manualmente antes de ejecutar el resto del script
\c edutech_db;

-- 3. Crear extensiones necesarias
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 4. Crear tablas en ORDEN CORRECTO (respetando dependencias)
-- ORDEN DE CREACIÓN:
-- 1. Tablas sin dependencias: campus, profesor, libro
-- 2. Tablas que dependen de las anteriores: facultad, programa, materia, aula, estudiante
-- 3. Tablas que dependen de las anteriores: horario, inscripcion, calificacion, prestamo

-- PASO 1: Tablas base (sin dependencias)
-- (Ejecutar CREATE TABLE para: campus, profesor, libro - ver secciones anteriores)

-- PASO 2: Tablas intermedias (dependen de PASO 1)
-- (Ejecutar CREATE TABLE para: facultad, programa, materia, aula, estudiante - ver secciones anteriores)

-- PASO 3: Tablas finales (dependen de PASO 2)
-- (Ejecutar CREATE TABLE para: horario, inscripcion, calificacion, prestamo - ver secciones anteriores)
